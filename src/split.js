...
// weight the split, selected by penaltiesInEffect:
//   - penalties ON  (default): weight by each character's xpSoFar (cumulative
//     XP earned to reach the current level; see src/character.js), which folds
//     in the combined race x class modifier.
...
import { assertBoolean } from "./validate.js";
...
/**
 * Split XP across a party's characters (level-1 members weighted as 1000).
 * @param {import("./party.js").Party} party a validated Party
 * @param {boolean} [penaltiesInEffect=true] when true, weight by xpSoFar (race
 *   x class modifier included); when false, weight by level only so same-level
 *   members split evenly regardless of race/class.
 * @returns {ReadonlyArray<Allocation>} per-character shares, in party order
 * @throws {RangeError} if party is not a Party or the flag is not a boolean.
 */
export function splitXp(party, penaltiesInEffect = true) {
