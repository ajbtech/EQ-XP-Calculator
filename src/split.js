...
/**
 * Split XP across a party's characters (level-1 members weighted as 1000). The
 * weighting method follows the party's penaltiesInEffect flag: on -> weight by
 * xpSoFar (race x class modifier included); off -> weight by level only so
 * same-level members split evenly regardless of race/class.
...
export function splitXp(party) {
  ...
  const { characters, penaltiesInEffect } = party;
