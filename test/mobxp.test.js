...
import { partyXpForMob } from "../src/mobxp.js";
...
test("solo, even con: base is mobLevel^2 * zem", () => {
  const party = makeParty([member(30)]);
...
test("applies the group bonus for party size", () => {
  const party = makeParty([member(30), member(30), member(30)]);
...
test("the consider modifier uses the HIGHEST level in the party", () => {
  ...
  const party = makeParty([member(10), member(30)]);
